...
import app.gaojinlei.note.constants.EditNoteConstans;
import app.gaojinlei.note.module.base.BasePresenter;
import app.gaojinlei.note.module.notes.Image.ImageInfoActivity;
import app.gaojinlei.note.module.notes.share.ShareActivity;
...
public class EditNotePresenter extends BasePresenter<IEditNoteView> implements IEditNotePresenter {
    ...
    public void initData() {
        ...
        notelog=intent.getStringExtra("note_log");//加载本便签的日志
        mView.setTitle(TimeUtils.millis2String(mModifiedTime));
    }

    public static String getNotelog() {
        return notelog==null||notelog.equals("")?"日志目前为空":notelog;
    }
...
//        屏幕宽
    ...
    public void saveNote(String content) {
        Intent intent = mView.getActivityIntent();
        // 内容改变时才保存
        if (!mNoteContent.equals(content)&&!content.equals(Data.getNewnotecontex())) {//新建的时候除去头和尾
